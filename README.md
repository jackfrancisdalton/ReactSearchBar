...
```
The value returned from this function will be used as the URL sent to fetch results from your server.
...
* qeuryResult : the server response from the query generated by `searchQueryURLFormatter`
...
This function must always return results in the form of an array of objects.
...
let resultMapper = function(RSB, queryResultJSON) {
	let formattedObjects = [];
...
let onClickButton = function(SBR, event, searchQuery, extraOptions) {
...
// ======= Defines the custom componenets to replace out-of-box components 
